...
MUST_REJECT = [
    ...
    ("bip borrowing tcp's protocol number", {"transport": "raw", "cipher": "auto", "raw_profile": "bip",
                                             "raw_proto": 6},
     "config.go: rawProtoBorrowed — bip writes no L4 header, so proto 6 is ciphertext where the TCP "
     "header belongs and the path drops the flow"),
    ("bip borrowing esp's protocol number", {"transport": "raw", "cipher": "auto", "raw_profile": "bip",
                                             "raw_proto": 50},
    ...
    ("spoof borrowing udp's protocol number", {"transport": "spoof", "cipher": "auto",
     ...
     "config.go: rawProtoBorrowed — spoof is bip-like and headerless, so it takes the same rule"),
