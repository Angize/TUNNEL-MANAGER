...
the core config in SECONDS, but the operator types them in MINUTES — so `tuningCard` divides on the way
...
function formOf(settings){
  const html = tuningCard(settings), vals = {};
...
def main():
    ...
    for fn in ("function tuningCard(", "function _collectTuning("):
        if fn not in js:
