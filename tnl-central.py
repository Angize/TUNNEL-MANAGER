...
function fillTargets(){var a=ssVal('c_a');selTargets={};var on=NODES.filter(function(n){return n.online&&n.id!=a});
 el('c_tgt_list').innerHTML=(on.length>10?'<input class="mssearch" placeholder="جستجو…" oninput="msFilter(this)">':'')+(on.length?on.map(function(n){return '<div class="msrow" data-id="'+n.id+'" onclick="tgtToggle(\\''+n.id+'\\')"><span class="mscheck"></span><span>'+esc(n.name)+'</span><span class="muted mono" style="font-size:11px;margin-inline-start:auto">'+esc(n.host)+'</span></div>'}).join(''):'<div class="muted" style="padding:11px 12px">نودِ آنلاینِ دیگری نیست</div>');
 updateTgt();renderSrcIp();renderTgtIps()}
function toggleTgtList(){var l=el('c_tgt_list');var open=l.style.display==='none';l.style.display=open?'block':'none';el('c_tgt_btn').classList.toggle('open',open)}
