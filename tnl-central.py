...
def _tunnel_extra(src, refetch_ech=True):
    ...
    if src.get("sni_split"):             # SNI fragmentation: split the wss ClientHello across TCP segments
        ...
        if src.get("sni_mode") == "disorder":   # low-TTL head desyncs a reassembling DPI
            e["sni_mode"] = "disorder"
...
def _sni_split_fields(d, cur):
    ...
    if sp:
        out["split_pos"] = sp
    # mode: "split" (دو سگمنتِ in-order) یا "disorder" (سگمنتِ سرْ با TTL پایین، ضدِ DPIِ بازسازی‌کننده)
    mode = str((d.get("sni_mode") if "sni_mode" in d else cur.get("sni_mode")) or "split").strip().lower()
    if mode not in ("split", "disorder"):
        raise ValueError("حالتِ SNI نامعتبر است (split یا disorder)")
    if mode == "disorder":
        out["sni_mode"] = "disorder"
...
function coreMeta(l){   // right col under box A, left col under box B (lock at the START, green)
 ...
 if(l.transport=='ws'&&l.ws_tls)feats.push('<span class="tag obfs">wss</span>');if(l.sni_split)feats.push('<span class="tag obfs">'+(l.sni_mode=='disorder'?'SNIdisorder':'SNIsplit')+'</span>');
...
function corToggleDisorder(){_corSniMode=(_corSniMode=='disorder')?'split':'disorder';var s=el('e_disorder');if(s)s.classList.toggle('on',_corSniMode=='disorder');var b=el('e_disorderbody');if(b)b.style.display=(_corSniMode=='disorder')?'':'none'}
...
function wsToggleRows(idp,fnp,tls,ech,sni,pos,mode,ttl,show){var hide=show?'':';display:none';
 return '<div class="tglbox" id="'+idp+'wstlsrow" style="margin-top:10px'+hide+'"><div class="tglsw'+(tls?' on':'')+'" id="'+idp+'wstls" onclick="'+fnp+'ToggleWsTls()"></div><div class="tt"><b>'+esc(T('wstls_t'))+'</b><small>'+esc(T('wstls_d'))+'</small></div></div>'
  ...
  +'<div class="tglbox" id="'+idp+'disorderrow" style="margin-top:8px"><div class="tglsw'+(mode=='disorder'?' on':'')+'" id="'+idp+'disorder" onclick="'+fnp+'ToggleDisorder()"></div><div class="tt"><b>'+esc(T('disorder_t'))+'</b><small>'+esc(T('disorder_d'))+'</small></div></div>'
  +'<div id="'+idp+'disorderbody" style="margin-top:6px'+((mode=='disorder')?'':';display:none')+'"><label>'+esc(T('sni_ttl_lbl'))+'</label><input id="'+idp+'splitttl" type="number" min="0" max="255" value="'+(ttl||0)+'"></div></div>';}
...
function ceToggleDisorder(){_eeSniMode=(_eeSniMode=='disorder')?'split':'disorder';var s=el('ee_disorder');if(s)s.classList.toggle('on',_eeSniMode=='disorder');var b=el('ee_disorderbody');if(b)b.style.display=(_eeSniMode=='disorder')?'':'none'}
