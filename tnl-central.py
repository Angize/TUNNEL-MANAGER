...
def _tunnel_extra(src, refetch_ech=True):
    ...
    if src.get("ws_no_sni"):             # no-SNI: omit the SNI extension (edge routes by the encrypted Host header)
        e["ws_no_sni"] = True
...
def _ws_fields(d, transport, cur=None):
    ...
    if ss:
        ...
        out.update(ss)
    # No-SNI: omit the SNI extension entirely, so an SNI-matching censor has nothing to match; the
    # edge routes by the (encrypted) Host header. An alternative to ECH for a CDN that serves
    # without SNI. wss only, and mutually exclusive with ECH and SNI fragmentation (both concern an
    # SNI there no longer is). Single-edge only — the pool branch returns before here.
    no_sni = d.get("ws_no_sni") if ("ws_no_sni" in d) else cur.get("ws_no_sni")
    if bool(no_sni):
        if not out.get("ws_tls"):
            raise ValueError("no-SNI به wss نیاز دارد — اول wss (TLS به CDN) را روشن کن")
        if out.get("ech"):
            raise ValueError("no-SNI و ECH با هم قابل‌استفاده نیستند (هر دو SNI را پنهان می‌کنند)")
        if out.get("sni_split"):
            raise ValueError("no-SNI با تقسیمِ SNI قابل‌استفاده نیست (دیگر SNIای برای شکستن نیست)")
        out["ws_no_sni"] = True
    return out
    ...
    with _reg_lock:
        ...
        for x in links:
            if x["id"] == L["id"]:
                ...
                for k in ("port", "psk", "cipher", "transport", "obfs", "cover", "cover_sni", "raw_profile", "flux_carrier", "flux_rotate_secs", "flux_shape", "flux_epoch_offset", "fec", "fec_data", "fec_parity", "ws_host", "ws_path", "ws_tls", "sni_split", "split_pos", "sni_mode", "split_ttl", "ws_no_sni", "ws_xhttp", "ws_xhttp_mode", "ech", "ws_ech", "edge_ip", "ws_pool", "ws_edge_ips", "ws_edge_ips_burned", "ws_edge_snis", "ws_edge_snis_burned", "ws_rotate_secs", "ws_auto_burn", "ws_warm_standby", "gso", "spoof_src", "spoof_dst", "fake_desync", "fake_ttl", "fake_count", "fake_mode"):   # keep only the extras this type uses; drop the rest
                    if k in extra:
                        x[k] = extra[k]
                    else:
                        x.pop(k, None)
...
(function(x){for(var k in x.fa)I18N.fa[k]=x.fa[k];for(var k in x.en)I18N.en[k]=x.en[k]})({fa:{
 ...
 wss_need_host:"برای wss باید دامنه (Host) را وارد کنی",ech_need_wss:"ECH به wss نیاز دارد — اول wss را روشن کن",sni_need_wss:"تقسیمِ SNI به wss نیاز دارد — اول wss را روشن کن",nosni_need_wss:"no-SNI به wss نیاز دارد — اول wss را روشن کن",
 ...
 wss_need_host:"For wss you must enter the domain (Host)",ech_need_wss:"ECH requires wss — turn on wss first",sni_need_wss:"SNI fragmentation requires wss — turn on wss first",nosni_need_wss:"No-SNI requires wss — turn on wss first",
...
(function(x){for(var k in x.fa)I18N.fa[k]=x.fa[k];for(var k in x.en)I18N.en[k]=x.en[k]})({fa:{
 ...
 ech_t:"ECH — مخفی‌کردنِ SNI",ech_d:"نامِ دامنه را داخلِ ClientHello رمز می‌کند تا فیلترچیِ SNI نبیند کدام دامنه است. نیازمندِ wss؛ برای استخر برای هر دامنه خودکار گرفته می‌شود.",sni_t:"تقسیمِ SNI (ضدِ DPI)",sni_d:"ClientHello را روی مرزِ دو بستهٔ TCP می‌شکند تا نامِ دامنه در یک بسته کامل نباشد و DPIِ SNI-محور نتواند تطبیق دهد. مکملِ ارزانِ ECH؛ نیازمندِ wss.",nosni_t:"بدونِ SNI (no-SNI)",nosni_d:"افزونهٔ SNI را کاملاً حذف می‌کند؛ لبهٔ CDN با هدرِ Host (رمزشده) مسیریابی می‌کند، پس DPIِ SNI-محور چیزی برای تطبیق ندارد. جایگزینِ ECH وقتی CDN بدونِ SNI سرویس می‌دهد؛ نیازمندِ wss. با ECH و تقسیمِ SNI هم‌زمان نمی‌شود.",sni_pos_lbl:"نقطهٔ برش (split_pos) — ۰ = خودکار (وسطِ دامنه)",disorder_t:"حالتِ disorder (ضدِ DPIِ بازسازی‌کننده)",disorder_d:"سگمنتِ اولِ ClientHello را با TTLِ پایین می‌فرستد تا در مسیر بمیرد و DPI بسته‌ها را بی‌ترتیب ببیند؛ کرنل با ارسالِ مجدد سرور را کامل می‌رساند. برای سانسورِ قوی‌تر که استریم را reassemble می‌کند.",sni_ttl_lbl:"TTLِ سگمنتِ سرْ (split_ttl) — ۰ = پیش‌فرض (۴)",sni_mode_lbl:"حالتِ تقسیم SNI",m_split_s:"دو سگمنتِ ساده",m_dis_s:"سگمنتِ سرْ با TTL پایین",m_fake_s:"ClientHelloِ جعلی (ضدِ reassembly)",
 // ws section
 ...
 ech_t:"ECH — hide the SNI",ech_d:"Encrypts the domain name inside the ClientHello so an SNI filter cannot see which domain it is. Requires wss; for a pool it is fetched automatically per domain.",sni_t:"SNI fragmentation (anti-DPI)",sni_d:"Splits the ClientHello across two TCP segments so no single packet holds the full domain name and an SNI-based DPI cannot match it. A cheap complement to ECH; requires wss.",nosni_t:"No-SNI",nosni_d:"Omits the SNI extension entirely; the CDN edge routes by the (encrypted) Host header, so an SNI-based DPI has nothing to match. An alternative to ECH when the CDN serves without SNI; requires wss. Not combinable with ECH or SNI fragmentation.",sni_pos_lbl:"Split position (split_pos) — 0 = auto (middle of the domain)",disorder_t:"disorder mode (anti-reassembly DPI)",disorder_d:"Sends the first ClientHello segment at a low TTL so it dies in transit and the DPI sees the packets out of order; the kernel retransmits it so the server still completes. For a stronger DPI that reassembles the stream.",sni_ttl_lbl:"Head-segment TTL (split_ttl) — 0 = default (4)",sni_mode_lbl:"SNI fragmentation mode",m_split_s:"two plain segments",m_dis_s:"low-TTL head",m_fake_s:"decoy ClientHello (anti-reassembly)",
...
function coreMeta(l){   // right col under box A, left col under box B (lock at the START, green)
 ...
 if(l.transport=='ws'&&l.ws_tls)feats.push('<span class="tag obfs">wss</span>');if(l.sni_split)feats.push('<span class="tag obfs">SNI'+(l.sni_mode||'split')+'</span>');if(l.ws_no_sni)feats.push('<span class="tag obfs">no-SNI</span>');
...
function corWsVis(){var ws=_corTr=='ws';var w=el('e_wsblk');if(w)w.style.display=ws?'':'none';var t=el('e_wstlsrow'),e=el('e_wsechrow');if(t)t.style.display=ws?'':'none';if(e)e.style.display=ws?'':'none';var sr=el('e_snisplitrow');if(sr)sr.style.display=ws?'':'none';var sb=el('e_snisplitbody');if(sb)sb.style.display=(ws&&_corSniSplit)?'':'none';var nr=el('e_nosnirow');if(nr)nr.style.display=ws?'':'none';if(ws){poolVis('e_');corWssGate()}}
function corToggleWsTls(){_corWsTls=!_corWsTls;var s=el('e_wstls');if(s)s.classList.toggle('on',_corWsTls);if(!_corWsTls){if(_corEch){_corEch=false;var e=el('e_wsech');if(e)e.classList.remove('on')}if(_corSniSplit){_corSniSplit=false;var q=el('e_snisplit');if(q)q.classList.remove('on');var b=el('e_snisplitbody');if(b)b.style.display='none'}if(_corNoSni){_corNoSni=false;var n=el('e_nosni');if(n)n.classList.remove('on')}}}
function corToggleSni(){if(!_corWsTls){_corSniSplit=false;var q=el('e_snisplit');if(q)q.classList.remove('on');alert(T('sni_need_wss'));return}_corSniSplit=!_corSniSplit;var s=el('e_snisplit');if(s)s.classList.toggle('on',_corSniSplit);var b=el('e_snisplitbody');if(b)b.style.display=_corSniSplit?'':'none';if(_corSniSplit&&_corNoSni){_corNoSni=false;var n=el('e_nosni');if(n)n.classList.remove('on')}}
function corToggleNoSni(){if(!_corWsTls){_corNoSni=false;var q=el('e_nosni');if(q)q.classList.remove('on');alert(T('nosni_need_wss'));return}_corNoSni=!_corNoSni;var s=el('e_nosni');if(s)s.classList.toggle('on',_corNoSni);if(_corNoSni){if(_corEch){_corEch=false;var e=el('e_wsech');if(e)e.classList.remove('on')}if(_corSniSplit){_corSniSplit=false;var q2=el('e_snisplit');if(q2)q2.classList.remove('on');var b=el('e_snisplitbody');if(b)b.style.display='none'}}}
...
function corToggleEch(){if(!_corWsTls){_corEch=false;var e=el('e_wsech');if(e)e.classList.remove('on');alert(T('ech_need_wss_alert'));return}_corEch=!_corEch;var s=el('e_wsech');if(s)s.classList.toggle('on',_corEch);if(_corEch&&_corNoSni){_corNoSni=false;var n=el('e_nosni');if(n)n.classList.remove('on')}}
var _poolData={};
...
function wsToggleRows(idp,fnp,tls,ech,sni,pos,mode,ttl,nosni,show){var hide=show?'':';display:none';
 return '<div class="tglbox" id="'+idp+'wstlsrow" style="margin-top:10px'+hide+'"><div class="tglsw'+(tls?' on':'')+'" id="'+idp+'wstls" onclick="'+fnp+'ToggleWsTls()"></div><div class="tt"><b>'+esc(T('wstls_t'))+'</b><small>'+esc(T('wstls_d'))+'</small></div></div>'
  ...
  +'<div class="tglbox" id="'+idp+'nosnirow" style="margin-top:9px'+hide+'"><div class="tglsw'+(nosni?' on':'')+'" id="'+idp+'nosni" onclick="'+fnp+'ToggleNoSni()"></div><div class="tt"><b>'+esc(T('nosni_t'))+'</b><small>'+esc(T('nosni_d'))+'</small></div></div>';}
...
async function openCoreModal(){var r=await j('node-names');NODES=r.nodes||[];var on=NODES.filter(function(n){return n.online});
 ...
 var b='<div class="grid2"><div><label class="first">'+esc(T('src_node'))+'</label>'+ssHTML('e_a',items,items[0].v,T('src_node'),'onCorNode')+'</div>'+
  ...
  wsToggleRows('e_','cor',false,false,false,0,'split',0,false,false)+
 ...
 corRoleLbls();renderCorIps();corCoverGate();corPortGate();corDesyncGate()}
...
function ceWsVis(){var ws=_eeTr=='ws';var w=el('ee_wsblk');if(w)w.style.display=ws?'':'none';var t=el('ee_wstlsrow'),e=el('ee_wsechrow');if(t)t.style.display=ws?'':'none';if(e)e.style.display=ws?'':'none';var sr=el('ee_snisplitrow');if(sr)sr.style.display=ws?'':'none';var sb=el('ee_snisplitbody');if(sb)sb.style.display=(ws&&_eeSniSplit)?'':'none';var nr=el('ee_nosnirow');if(nr)nr.style.display=ws?'':'none';if(ws){poolVis('ee_');ceWssGate()}}
function ceToggleWsTls(){_eeWsTls=!_eeWsTls;var s=el('ee_wstls');if(s)s.classList.toggle('on',_eeWsTls);if(!_eeWsTls){if(_eeEch){_eeEch=false;var e=el('ee_wsech');if(e)e.classList.remove('on')}if(_eeSniSplit){_eeSniSplit=false;var q=el('ee_snisplit');if(q)q.classList.remove('on');var b=el('ee_snisplitbody');if(b)b.style.display='none'}if(_eeNoSni){_eeNoSni=false;var n=el('ee_nosni');if(n)n.classList.remove('on')}}}
function ceToggleSni(){if(!_eeWsTls){_eeSniSplit=false;var q=el('ee_snisplit');if(q)q.classList.remove('on');alert(T('sni_need_wss'));return}_eeSniSplit=!_eeSniSplit;var s=el('ee_snisplit');if(s)s.classList.toggle('on',_eeSniSplit);var b=el('ee_snisplitbody');if(b)b.style.display=_eeSniSplit?'':'none';if(_eeSniSplit&&_eeNoSni){_eeNoSni=false;var n=el('ee_nosni');if(n)n.classList.remove('on')}}
function ceToggleNoSni(){if(!_eeWsTls){_eeNoSni=false;var q=el('ee_nosni');if(q)q.classList.remove('on');alert(T('nosni_need_wss'));return}_eeNoSni=!_eeNoSni;var s=el('ee_nosni');if(s)s.classList.toggle('on',_eeNoSni);if(_eeNoSni){if(_eeEch){_eeEch=false;var e=el('ee_wsech');if(e)e.classList.remove('on')}if(_eeSniSplit){_eeSniSplit=false;var q2=el('ee_snisplit');if(q2)q2.classList.remove('on');var b=el('ee_snisplitbody');if(b)b.style.display='none'}}}
...
function ceToggleEch(){if(!_eeWsTls){_eeEch=false;var e=el('ee_wsech');if(e)e.classList.remove('on');alert(T('ech_need_wss_alert'));return}_eeEch=!_eeEch;var s=el('ee_wsech');if(s)s.classList.toggle('on',_eeEch);if(_eeEch&&_eeNoSni){_eeNoSni=false;var n=el('ee_nosni');if(n)n.classList.remove('on')}}
...
function openCoreEdit(id){var l=FLEET.filter(function(x){return x.id==id})[0];if(!l){toast(T('not_found'),'err');return}
 ...
 var b='<div class="muted" style="font-size:12px;margin-bottom:10px">'+esc(l.a_name)+' ↔ '+esc(l.b_name)+' · <span class="mono">'+esc(l.name)+'</span></div>'+
  ...
  wsToggleRows('ee_','ce',_eeWsTls,_eeEch,_eeSniSplit,_eeSplitPos,_eeSniMode,_eeSplitTtl,_eeNoSni,_eeTr=='ws')+
