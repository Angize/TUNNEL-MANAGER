...
b = tnl._port_bindings("core", 443, "tcp", "a", 50, A, B)
check("core checks only the server node (a), proto=tcp", b == [(A, 443, "tcp")])
b = tnl._port_bindings("core", 443, "tcp", "b", 50, A, B)
check("core server_side=b -> node B", b == [(B, 443, "tcp")])
b = tnl._port_bindings("core", None, None, "a", 50, A, B)
check("core default port = 20000+id, proto defaults udp", b == [(A, 20050, "udp")])
...
try:
    tnl._create_tunnel_impl({"a_node": "na", "b_node": "nb", "type": "core",
                             "server_side": "a", "transport": "tcp", "port": 443, "cipher": "auto"})
    check("core create with busy server port -> blocked", False)
except ValueError as e:
    msg = str(e)
    check("core create with busy server port -> blocked", "443/TCP" in msg and "NodeA" in msg and "xray" in msg)

# ---- create: client-side busy is IGNORED (client never binds) ----------------
install(busy={("nb", 443, "tcp")})  # B is the client here (server_side=a)
r = tnl._create_tunnel_impl({"a_node": "na", "b_node": "nb", "type": "core",
                             "server_side": "a", "transport": "tcp", "port": 443, "cipher": "auto"})
check("core create ignores busy client port", r.get("ok") is True)
...
check("core create on free default port succeeds", r.get("ok") is True)
...
LINKS[:] = [LINK]
r = tnl._edit_link_impl({"id": "L1", "type": "core", "server_side": "a",
...
LINKS[:] = [LINK]
try:
    tnl._edit_link_impl({"id": "L1", "type": "core", "server_side": "a",
